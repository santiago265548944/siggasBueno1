...
export class SeekPredioComponent implements OnInit {
   ...
   constructor(
      ...
      private identidaPredio: IdentidaPredioService
   ) {
   ...
   private executeReturnElementAction(returnElementAction: ReturnElementAction): void {
      if (returnElementAction.EMapAction === EmapActions.SeekPredioGeometry) {
         loadModules(
            ...
            const iTask = new IdentifyTask(
               genericUrl + this.memoryService.getItem('PredioServiceName') + '/MapServer'
            );
            iTask.execute(
               identifyParameters,
               (identifyResults) =>
                  this.identidaPredio.onIdentifyTaskComplete(identifyResults, identifyParameters),
   ...
   private createIdentifyParameters(
      ...
      identifyParameters.layerOption = IdentifyParametersClass.LAYER_OPTION_VISIBLE;
      identifyParameters.spatialReference = returnElementAction.map.spatialReference;
      identifyParameters.layerIds = new Array<number>();
      identifyParameters.tolerance = 1;
      identifyParameters.returnGeometry = true;
      identifyParameters.layerIds = [this.memoryService.getItem('PredioLayerIndex')];
      return identifyParameters;
   }

   onIdentifyTaskComplete(identifyResultsArg: Array<any>, identifyParameters: any): void {
      console.log('Identify Results in Service:', identifyResultsArg);
      ...
      if (identifyResultsArg.length > 0) {
         ...
         this.domPredio = this.createDOMPredio(identifyResultsArg[0].feature.attributes);
