...
export class ConsultarOrdenesComponent implements OnInit {
  ...
  buscarOrdenes(numeroOrden: string) {
    if (this.buscarOrden && this.numeroOrden) {
      ...
      const params = [new InputParameter("una_orden", this.numeroOrden)];
      ...
      this.apiService
        ...
        .subscribe((response) => {
          ...
          if (parseInt(this.codigoerror) !== 0) {
            console.log(
              "Se recibió un código de error diferente de 0:",
              this.codigoerror
            );
            alert(
              "Se ha producido un error. Código de error: " + this.codigoerror
            );
  ...
  Histocambioorden() {
    if (this.buscarOrden && this.numeroOrden) {
      ...
      this.apiService
        .callStoreProcedureV2(
          RequestHelper.getParamsForStoredProcedureV2(
            StoreProcedures.HistocambiosOrden,
            [new InputParameter("una_orden", this.numeroOrden)]
          )
        )
        .subscribe((response) => {
          //  console.log(
          //    "Respuesta del procedimiento histocambioorden:",
          //    response
          //  );
          ...
          try {
            ...
            // Si la lista de cambios sigue vacía después de procesar la respuesta, mostrar la alerta de error
            if (this.listaHistorialCambios.length === 0) {
              alert("No se encontraron cambios para la orden.");
            }
          } catch (error) {
  ...
  validarFechas() {
    ...
    if (this.fechaIniEje != "01-JAN-00" && this.fechaFinEje != "01-JAN-00") {
      const fechaEquipo = moment(
        this.fechaIniEje,
        "MM/DD/YYYY HH:mm:ss"
      ).format("MM/DD/YYYY HH:mm:ss");
      const fechaEquipo1 = moment(
        this.fechaFinEje,
        "MM/DD/YYYY HH:mm:ss"
      ).format("MM/DD/YYYY HH:mm:ss");
  ...
  getOrderTags() {
    ...
    let buscaNumeroOrden = this.buscarOrden
      ? this.numeroOrden
      : this.orderSelected;
    ...
    this.apiService
      ...
      .subscribe(
        ...
        (error) => {
          ...
          alert(
            "La solicitud ha excedido el tiempo límite. Intente nuevamente más tarde."
          );
        }
      );
  }
