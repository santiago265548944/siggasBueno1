...
export class SelectionComponent implements OnInit, OnDestroy {
   ...
   onMapServiceChange(newMapService: any): void {
      if (newMapService != null) {
         ...
         // Emitir la función de cambio del select al servicio
         const selectChangeFunction = (event: any) => {
            const selectedValue = event.target.value;
            this.dataSharing.setSelectedSelectValue(selectedValue);
            console.log('Valor del select cambiado:', event.target.value);
         };
         this.dataSharing.setSelectChangeFunction(selectChangeFunction);
      }
   }
   onSelectChange(event: any): void {
      // Obtener la función de cambio del select desde el servicio
      const selectChangeFunction = this.dataSharing.getSelectChangeFunction();

      // Ejecutar la función si está definida
      if (selectChangeFunction) {
         selectChangeFunction(event);
      }
   }

   fillSelectLayers(mapService: any): void {
      this.layerIdSelected = null;
      const parentLayerInfos = _.filter(mapService.layerInfos, (layerInfo: any) => {
   ...
   createLayersGroup(parentLayerInfo: any, layersInfo: any, depth: number): void {
   ...
   private initSelect(geometry: any) {
      // console.log('LayerIdSelected before initSelect:', this.layerIdSelected);
      ...
      loadModules(
         ...
         { url: 'https://js.arcgis.com/3.23/' }
      ).then(([FeatureLayer, SimpleMarkerSymbol, Color, SimpleLineSymbol, SimpleFillSymbol]) => {
         const url =
         ...
         this.globals.getFeatureMapAdded().on('load', (evt) => {
            ...
            switch (this.globals.getFeatureMapAdded().geometryType) {
               ...
               case 'esriGeometryPolygon':
                  symbol = this.createSimpleFillSymbol(SimpleFillSymbol, SimpleLineSymbol, Color);
                  break;
            ...
            this.globals.getFeatureMapAdded().on('selection-clear', () => this.onSelectionClear());
            this.globals
               .getFeatureMapAdded()
               .on('selection-complete', (eventArg) => this.onSelectionComplete(eventArg));
   ...
   onSelectionComplete(eventArg): void {
      const selectedFeatures = this.globals.getFeatureMapAdded().getSelectedFeatures();

      if (selectedFeatures.length > 0) {
         this.dataSharing.setSelectedFeatures(selectedFeatures);
      }
   ...
   private performViewSelectionResponse(): void {
      ...
      // console.log('id', this.layerIdSelected);
   }
   ...
   private selectFeatures(geometry: void): void {
      loadModules(['esri/layers/FeatureLayer', 'esri/tasks/query'], {
         ...
         this.globals.getFeatureMapAdded().selectFeatures(selectQuery, FeatureLayer.SELECTION_NEW);
      });
   }
   ...
   private createSimpleFillSymbol(SimpleFillSymbol: any, SimpleLineSymbol: any, Color: any): any {
      return new SimpleFillSymbol(
         SimpleFillSymbol.STYLE_SOLID,
         new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color('rgb(0,0,255)'), 1),
